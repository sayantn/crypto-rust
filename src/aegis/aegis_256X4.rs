...
type State = [AesBlockX4; 6];

fn state_update(state: &mut State, m: AesBlockX4) {
    let temp = state[5];
...
fn encrypt_one_block(state: &mut State, plaintext: &[u8; 64], ciphertext: &mut [u8; 64]) {
    let p = AesBlockX4::from(plaintext);
...
fn decrypt_one_block(state: &mut State, ciphertext: &[u8; 64], plaintext: &mut [u8; 64]) {
    let p = AesBlockX4::from(ciphertext) ^ state[1] ^ state[4] ^ state[5] ^ (state[2] & state[3]);
...
fn decrypt_last_block(state: &mut State, ciphertext: &[u8], position: usize) -> [u8; 64] {
    ...
    let p = AesBlockX4::from(temp) ^ state[1] ^ state[4] ^ state[5] ^ (state[2] & state[3]);
    p.store_to(&mut temp);
...
aegis_impl!(Aegis256X4Enc, Aegis256X4Dec, 64);
